...
def main():
    ...
    # Get total orders
    response = requests.get(f"https://api.mercadolibre.com/orders/search?seller=429996719&order.status=paid&order.date_created.from={date_from_strf}&sort=date_desc", headers=header)
    while response.status_code == 500 or response.status_code == 403:
        sleep(10)
        print('Retrying get total orders...')
        response = requests.get(f"https://api.mercadolibre.com/orders/search?seller=429996719&order.status=paid&order.date_created.from={date_from_strf}&sort=date_desc", headers=header)
    ...
    while offset < total_orders:
        ...
        offset += 51
        response = requests.get(f"https://api.mercadolibre.com/orders/search?seller=429996719&order.status=paid&order.date_created.from={date_from_strf}&offset={offset}&sort=date_desc", headers=header)
        while response.status_code == 500 or response.status_code == 403:
            sleep(10)
            print('Retrying consulta todos os pedidos e consulta...')
            response = requests.get(f"https://api.mercadolibre.com/orders/search?seller=429996719&order.status=paid&order.date_created.from={date_from_strf}&offset={offset}&sort=date_desc", headers=header)
        response = response.json()
    ...
    # Exporta em excel
    name_file_excel = 'margem_mercadolivreRedPlace_' + str(hoje) + '.xlsx'
