...
def main(file, empresa_personalizada):
    ...
    AND B.ORIGEM = {empresa_personalizada}
    '''
    ...
    # Planilha full
    df_magalu_full = pd.read_excel(file)

    df_magalu_full['subtracao'] = df_magalu_full['Programado'] + \
                            df_magalu_full['Bloqueado'] + \
                            df_magalu_full['Em trânsito']
                            
    df_magalu_full_filtrada = df_magalu_full[['Código referência do produto', 'Código Magalu do produto', 'Venda últimos 30 dias', 'À venda', 'Esgota em', 'subtracao']]

    # Renomeando as colunas
    novos_nomes_colunas = {'Código referência do produto':'COD_ML', 'Código Magalu do produto':'ID_ANUNCIO', 'Venda últimos 30 dias':'VENDAS_30', 'À venda':'APTAS_FULL', 'Esgota em':'TEMPO'}
    df_magalu_full_filtrada.rename(columns=novos_nomes_colunas, inplace=True)


    # Junta as duas planilhas
    df_completo = pd.merge(data_completo, df_magalu_full_filtrada, on='COD_ML', how='left')
    ...
    df_magalu_full.to_excel(writer, sheet_name='PLAN_FULL', index=False)
